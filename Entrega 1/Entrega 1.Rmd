...
> \small Se tomaron 81 corrimientos a intervalos regulares desde 1 hasta 3 por lo que una tabla no nos daría mucha información a simple vista sobre el comportamiento de los ARL. Comparando el ARL de la carta $R$ y la carta $S$ nos damos cuenta que para $n = 3$ a la carta $R$ le toma, en promedio, más tiempo para producir falsas alarmas cuando el proceso aún es estable mientras que para los corrimientos de 1.1 veces la desviación estándar en adelante, la carta $R$ también tiende a tardar menos en dar una alarma verdadera. Por otro lado, la carta $S$ al fijar el tamaño de los subgrupos racionales en 10 toma mucho más tiempo que la carta $R$ para dar falsas alarmas y se ve un menor tiempo promedio para dar verdaderas alarmas a partir de corrimientos de 1.15 veces la desviación estándar aunque no se observa una diferencia tan marcada para corrimientos más adelante. Ahora revisemos cómo cambian las cartas con respecto a ellas mismas:
...
abline(v = seq(1, 3, by = 0.1), col = 'gray', lwd = 0.8, lty = 'dashed')
mtext(bquote(bold('El proceso estable N(0,1)')), side = 3, line = 0.5,
...
> \small Vemos que la carta $R$ tiene un comportamiento más estable respecto al cambio del tamaño de los subgrupos racionales mientras que S tiende a comportarse mejor a medida que este aumenta, resultando en que esta última tiene un comportamiento más deseado para muestras más grandes pues va mejorando, tanto en menor frecuencia de falsas alarmas como mayor frecuencia de alarmas verdaderas, a medida que el tamaño aumenta.
