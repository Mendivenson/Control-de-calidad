...
plot(x = NULL, y = NULL, ylim = c(0,1), xlim = c(1,6),main = TeX('\\textbf{Curvas OC para carta} ${S^2}$'),
     xlab = TeX('Corrimiento ($\\sigma^2_1 = k \\times \\sigma_0^2$)'),ylab = TeX('$\\beta$'), cex.axis = 0.8,
     cex.lab = 0.7)
...
#  muestra num11 en adelante con cambio en la media del proceso
muestra_num <- 11
set.seed(13)
while (TRUE) {
  muestra <- generar_muestra(mu_0 + k * sigma_0, sigma_0, n)
  media_muestra <- mean(muestra)
  muestras[[muestra_num]] <- muestra
...
> Finalmente el momento en que más probablemente se dio el cambio en la media resultará del estimador $\hat{\tau}_{MV} = argmax_{0 \leq t < T} \left[(T - t) (\bar{\bar{X}}_{T,t} - \mu_0)^2\right]$ implementado en R así:
...
for (i in 0:(Total - 1)){
  ...
  FV = c(FV,(Total-i) * (media_no_control - mu_0)^2)
}
```
...
plot(x = 0:(length(medias) - 1),y = FV,type = 'b', cex = 0.8,
     main = TeX('$(T - t) (\\bar{\\bar{X}}_{T,t} - \\mu_0)^2$'), ylab = TeX('$\\tau$'), xlab = 'Tiempo')
